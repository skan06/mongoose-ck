...
// 4 CRUD's creation
/* get:test
 * path:http://localhost:1200/api/contacts/test 
*/ 
router.get("/test",(req,res)=>{
    res.send("hello f3")
})
/* post:add
 * path:http://localhost:1200/api/contacts/add
 * req.body 
*/ 
router.post("/add",async(req,res)=>{
    ...
    if (!name.length || !email.length) {
        res.status(400).send({msg:"You should add name and email"})
}
    ...
    try {
        ...
        res.status(400).send({msg:"No contact added",error})
    }
})
/* post:get all
 * path:http://localhost:1200/api/contacts/
 *  
*/ 
...
/* post:get one
 * path:http://localhost:1200/api/contacts/:_id
 * req.params 
*/ 
...
/* post:update
 * path:http://localhost:1200/api/contacts/edit/:_id
 * req.params && req.body
*/ 
...
/* post:delete
 * path:http://localhost:1200/api/contacts/delete/:_id
 * req.params
*/ 
